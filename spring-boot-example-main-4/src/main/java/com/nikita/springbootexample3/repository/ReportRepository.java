...
public interface ReportRepository extends JpaRepository<Report,Integer> {
    ...
    @Query("SELECT new com.nikita.springbootexample3.dto.AppResponse(p.name,p.age,r.test,r.status,d.dname) FROM Report r JOIN r.patient p JOIN r.doctor d")
    ...
    @Query("SELECT new com.nikita.springbootexample3.dto.Join(r.id, r.test, p.pid, p.name, p.age) "
            + "FROM Report r RIGHT JOIN r.patient p")
